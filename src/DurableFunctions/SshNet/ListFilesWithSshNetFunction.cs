...
public class ListFilesWithSshNetFunction(SshNetSftp sshNetSftp, ILogger<ListFilesWithSshNetFunction> logger)
{
    private static readonly string SftpDirectory = Environment.GetEnvironmentVariable(nameof(SftpDirectory))!;
    
    [Function(nameof(ListFilesWithSshNetFunction))]
    public Task Run([ActivityTrigger] TaskActivityContext? context = null)
    {
        try
        {
            ...
            var files = sshNetSftp.ListFiles(SftpDirectory);
